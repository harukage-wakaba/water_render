Shader "Custom/Caustics"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 getRipplesNormal(float3 water_pos, float3 water_tangent, float2 texcoord_xy)
            {
                float3 flat_normal = float3(0.0, 1.0, 0.0);

                float3 pos = ripples(water_pos, getAdjUV(water_pos));
                float3 tangent = water_tangent;
                float3 binormal = normalize(cross(flat_normal, tangent));

                float delta = 0.00390625 * 1.0;
                float3 posT = ripples(water_pos + tangent * delta, getAdjUV(water_pos) + float2(delta, 0.0));
                float3 posB = ripples(water_pos + binormal * delta, getAdjUV(water_pos) + float2(0.0, delta));
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 wave_ddx_normal = getRipplesNormal(wave_ddx_flat_pos, i.tangent,i.uv);
                ...
                float3 wave_ddy_normal = getRipplesNormal(wave_ddy_flat_pos, i.tangent,i.uv);
                ...
                // 面積の差
                float caustics_rate = max(flat_area / wave_area,0.85);
                caustics_rate = min(caustics_rate,2.0);

                caustics_rate *= 1.0;

                // caustics_rate = i.rePos.y + 0.0260;
                // caustics_rate *= 34.0;

                col += float4(caustics_rate, caustics_rate, caustics_rate, caustics_rate);
                ...
                */
                ...
                // 角度オフセット
                float check_offset_deg = 4.0;

                float check_offset_deg_table[10] = { 0.0,1.0,2.0,5.0,9.0,14.0,20.0,27.0,35.0,44.0 };

                // 角度オフセットZ
                float check_offset_deg_z = 8.0;
                ...
                for( int z = 0; z < 45; z++ )
                {
                    for ( int x = 0; x <10; x++ )
                    {
                        ...
                        float check_deg = check_strat_deg_x + check_offset_deg_table[x];
                        ...
                        float3 wave_on_normal = getRipplesNormal(wave_on_pos, tan, i.uv);
